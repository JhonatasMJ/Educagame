...
const StartPhase = ({
  ...
  const [tips, setTips] = useState<{ title: string; content: string } | null>(
    null
  );
      ...
      <View className="flex-1">
        ...
        <View className="w-full">
          <View className="w-full bg-secondary py-10 border-b-4 border-tertiary items-center justify-center">
            <ArrowBack
              color="#fff"
              size={20}
              className="absolute bg-tertiary top-2 left-2"
            />

            <Text className="text-3xl font-bold text-white text-center pt-2 px-5">
              {title}
            </Text>
          </View>
          ...
          {subTitle && (
            <View className="w-full bg-[#31C7ED] py-3 items-center justify-center border-b-2 border-[#28a8c9]">
              <Text className="text-xl font-semibold text-white text-center px-5">
                {subTitle}
              </Text>
            </View>
          )}
            ...
            <View className="flex-1 w-full px-6 items-center my-4">
              ...
              {tips && (
                <View className="w-full bg-[#FFF8E1] p-5 rounded-xl mb-5 border border-[#FFE082]">
                  <Text className="text-lg font-bold text-[#F57C00] mb-2">
                    {tips.title}
                  </Text>
                  ...
                  {keyPoints.map((point, index) => (
                    <View key={index} className="flex-row items-start mt-2">
                      <CheckCircle
                        ...
                        className="mt-0.5"
                      />
                      <Text className="text-base text-gray-700 ml-2 flex-1">
                        {point}
                      </Text>
                    </View>
                  ))}
              ...
              {image && (
                <View className="w-full items-center mb-5 rounded-xl overflow-hidden shadow-lg">
                  <Image
                    source={{ uri: image }}
                    className="w-full rounded-xl h-52"
                    resizeMode="cover"
                  />
                </View>
              )}
        ...
        <View className="bottom-0 left-0 right-0 px-6 py-4 bg-primary border-t border-gray-200">
          <View className="relative">
            <CustomButton
              title="CONTINUAR"
              nextStep={`/questions/game?phaseId=${phaseId}${
